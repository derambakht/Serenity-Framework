...
import {
    ...
    RemoteView,
    RemoteViewOptions,
} from "@serenity-is/corelib/slick";
import {
    AutoTooltips,
    Column,
    ColumnSort,
...
export class DataGrid<TItem, TOptions>
    extends Widget<TOptions>
    implements IDataGrid, IReadOnly
{
    ...
    constructor(container: JQuery, options?: TOptions) {
        ...
        var layout = function () {
            self.layout();
            if (self._layoutTimer != null) LayoutTimer.store(self._layoutTimer);
        };
        this.element
            .addClass("require-layout")
            .on("layout." + this.uniqueName, layout);

        if (this.useLayoutTimer())
            this._layoutTimer = LayoutTimer.onSizeChange(
        ...
        // @ts-ignore
        this.view.setCardConfig(options?.cardConfig);

        if (this.useAsync()) this.initAsync();
        else this.initSync();
    }
    ...
    protected internalInit() {
        ...
        // @ts-ignore
        if (this.options?.cardConfig?.showCard) {
            $(".card-container").show();
            $(".grid-container").hide();
        }
    ...
    protected layout(): void {
        ...
        var madeAutoHeight =
            this.slickGrid != null && this.slickGrid.getOptions().autoHeight;
    ...
    protected ensureQuickFilterBar(): QuickFilterBar {
        if (this.quickFiltersDiv == null) this.createQuickFilters([]);
    ...
    protected createQuickFilters(
        ...
        if (
            ...
            if (this.toolbar) {
                ...
                this.quickFiltersDiv.appendTo(
                    $("<div/>")
                        .addClass("s-Toolbar")
                        .insertBefore(this.slickContainer)
                );
            }

            this.quickFiltersBar = new QuickFilterBar(this.quickFiltersDiv, {
                filters: filters,
                getTitle: (filter: QuickFilter<Widget<any>, any>) =>
                    this.determineText((pre) => pre + filter.field),
    ...
    public static propertyItemToQuickFilter(item: PropertyItem) {
        ...
        if (filteringType === DateFiltering) {
            ...
            quick = QuickFilterBar.dateTimeRange(
                name,
                title,
                item.editorParams?.useUtc
            );
            ...
            if (filtering && isInstanceOfType(filtering, IQuickFiltering)) {
                ...
                quick.options = extend(
                    deepClone(quick.options),
                    item.quickFilterParams
                );
            } else {
                return null;
            }
    ...
    protected findQuickFilter<TWidget>(
        type: { new (...args: any[]): TWidget },
        field: string
    ): TWidget {
        if (this.quickFiltersBar != null)
            return this.quickFiltersBar.find(type, field);

        return $("#" + this.uniqueName + "_QuickFilter_" + field).getWidget(
            type
        );
    }

    protected tryFindQuickFilter<TWidget>(
        type: { new (...args: any[]): TWidget },
        field: string
    ): TWidget {
        if (this.quickFiltersBar != null)
    ...
    protected createIncludeDeletedButton(): void {
        if (
            !isEmptyOrNull(this.getIsActiveProperty()) ||
            !isEmptyOrNull(this.getIsDeletedProperty())
        ) {
            GridUtils.addIncludeDeletedToggle(
    ...
    protected initialPopulate(): void {
        ...
        if (this.populateWhenVisible()) {
            LazyLoadHelper.executeEverytimeWhenShown(
                this.element,
                function () {
    ...
    protected initializeFilterBar() {
        ...
        this.filterBar
            .get_store()
            .add_changed((s: JQueryEventObject, e: any) => {
    ...
    protected createSlickGrid(): Grid {
        var visibleColumns = this.postProcessColumns(this.allColumns).filter(
            function (x) {
    ...
    protected onClick(e: JQueryEventObject, row: number, cell: number): void {
        ...
        if (target.hasClass("s-EditLink")) {
            e.preventDefault();
            this.editItemOfType(
                SlickFormatting.getItemType(target),
                SlickFormatting.getItemId(target)
            );
        }
    }
    ...
    protected createSlickContainer(): JQuery {
        $(
            '<div class="card-container row" style="display:none"></div>'
        ).appendTo(this.element);
        return $('<div class="grid-container my-2"></div>').appendTo(
            this.element
        );
    }
        ...
        if (this.slickGrid) {
            ...
            if (columns.length > 0) {
                columns.sort(function (x1, y) {
                    return Math.abs(x1.sortOrder) < Math.abs(y.sortOrder)
                        ? -1
                        : Math.abs(x1.sortOrder) > Math.abs(y.sortOrder)
                        ? 1
                        : 0;
                });
    ...
    protected createToolbar(buttons: ToolButton[]): void {
        var toolbarDiv = $('<div class="grid-toolbar"></div>').appendTo(
            this.element
        );
    ...
    setTitle(value: string) {
        if (value !== this.getTitle()) {
            if (value == null) {
                ...
                if (!this.titleDiv) {
                    this.titleDiv = $(
                        '<div class="grid-title"><div class="title-text"></div></div>'
    ...
    protected itemLink(
        itemType?: string,
        idField?: string,
        text?: (ctx: FormatterContext) => string,
    ...
    protected getPropertyItemsData(): PropertyItemsData {
        ...
        if (
            this.getColumnsKey === DataGrid.prototype.getColumnsKey &&
    ...
    protected propertyItemsToSlickColumns(
        ...
        for (var i = 0; i < propertyItems.length; i++) {
            ...
            if (item.editLink === true) {
                ...
                var css = {
                    $:
                        item.editLinkCssClass != null
                            ? item.editLinkCssClass
                            : null,
                };
                column.format = this.itemLink(
                    item.editLinkItemType != null
                        ? item.editLinkItemType
                        : null,
            ...
            } else if (item.formatterType === "Date") {
                column.format = (ctx) =>
                    !!ctx.value
                        ? new Intl.DateTimeFormat("fa-IR", {
                              // @ts-ignore
                              numberingSystem: "latn",
                          }).format(new Date(ctx.value))
                        : ctx.value;
            }
        }
        return columns;
    ...
    protected getLocalTextDbPrefix(): string {
        if (this._localTextDbPrefix != null) return this._localTextDbPrefix;

        this._localTextDbPrefix = this.getLocalTextPrefix() ?? "";
        if (
            this._localTextDbPrefix.length > 0 &&
            !endsWith(this._localTextDbPrefix, ".")
        )
    ...
    protected getLocalTextPrefix(): string {
        ...
        if (rowDefinition) return rowDefinition.localTextPrefix;

        var attr = this.attrs(LocalTextPrefixAttribute);

        if (attr.length >= 1) return attr[0].value;
    }

    private _idProperty: string;

    protected getIdProperty(): string {
        if (this._idProperty != null) return this._idProperty;
        ...
        if (attr.length === 1) return (this._idProperty = attr[0].value ?? "");
    ...
    protected getIsActiveProperty(): string {
        if (this._isActiveProperty != null) return this._isActiveProperty;

        var rowDefinition = this.getRowDefinition();
        if (rowDefinition)
            return (this._isActiveProperty =
                rowDefinition.isActiveProperty ?? "");
    ...
    protected updateDisabledState(): void {
        this.slickContainer.toggleClass(
            "ui-state-disabled",
            !!this._isDisabled
        );
    }
    ...
    protected addQuickFilter<TWidget extends Widget<any>, TOptions>(
    ...
    protected restoreSettings(
        ...
        try {
            ...
            if (settings.columns != null) {
                ...
                if (flags.sortColumns !== false) {
                    ...
                    this.view.sortBy = list.map(function (x5) {
                        return (
                            x5.columnId + (x5.sortAsc === false ? " DESC" : "")
                        );
                    });
            ...
            if (
                settings.includeDeleted != null &&
                flags.includeDeleted !== false
            ) {
                var includeDeletedToggle = this.element.find(
                    ".s-IncludeDeletedToggle"
                );
                if (
                    !!settings.includeDeleted !==
                    includeDeletedToggle.hasClass("pressed")
                ) {
            ...
            if (
                ...
                this.quickFiltersDiv.find(".quick-filter-item").each((i, e) => {
                    ...
                    var widget = $(
                        "#" + this.uniqueName + "_QuickFilter_" + field
                    ).tryGetWidget(Widget);
                    if (widget == null) {
                        return;
                    }
            ...
            if (
                flags.quickSearch === true &&
                (settings.quickSearchField !== undefined ||
                    settings.quickSearchText !== undefined)
            ) {
                var qsInput = this.toolbar.element
                    .find(".s-QuickSearchInput")
                    .first();
                if (qsInput.length > 0) {
                    var qsWidget = qsInput.tryGetWidget(QuickSearchInput);
                    qsWidget &&
                        qsWidget.restoreState(
                            settings.quickSearchText,
                            settings.quickSearchField
                        );
                }
            }
        } finally {
    ...
    protected getCurrentSettings(flags?: GridPersistanceFlags) {
        ...
        if (
            ...
            for (var column of columns) {
                ...
                if (flags.sortColumns !== false) {
                    ...
                    p.sort =
                        sort >= 0
                            ? sortColumns[sort].sortAsc !== false
                                ? sort + 1
                                : -sort - 1
                            : 0;
                }
        ...
        if (flags.includeDeleted !== false) {
            settings.includeDeleted = this.element
                .find(".s-IncludeDeletedToggle")
                .hasClass("pressed");
        }
        ...
        if (
            ...
            settings.filterItems = this.filterBar
                .get_store()
                .get_items()
                .slice();
        }
        ...
        if (
            ...
            this.quickFiltersDiv.find(".quick-filter-item").each((i, e) => {
                ...
                var state =
                    saveState != null
                        ? saveState(widget)
                        : EditorUtils.getValue(widget);
                settings.quickFilters[field] = state;
                if (
                    flags.quickFilterText === true &&
                    $(e).hasClass("quick-filter-active")
                ) {
                    ...
                    if (getDisplayText != null) {
                        ...
                        displayText =
                            filterLabel +
                            " = " +
                            EditorUtils.getDisplayText(widget);
                    }

                    if (!isEmptyOrNull(displayText)) {
                        if (!isEmptyOrNull(settings.quickFilterText)) {
                            settings.quickFilterText +=
                                " " +
                                (tryGetText("Controls.FilterPanel.And") ??
                                    "and") +
                                " ";
