...
export class DateEditor extends Widget<any> implements IStringValue, IReadOnly {
    ...
    constructor(input: JQuery) {
        ...
        if (typeof flatpickr !== "undefined" && (DateEditor.useFlatpickr || !$.fn.datepicker)) {
        ...
        else if (($.fn as any)?.datepicker && false) {
            ...
            const jalaliId = input.attr("id") + "-jalali";
            ...
            setTimeout(() => {
              ...
              if(value){
                  $shamsiInput.val(new Intl.DateTimeFormat('fa-IR', {
                    // @ts-ignore
            ...
            let $shamsiInput = $(`<input id='${jalaliId}'>`);
            if(defaultValue){
                $shamsiInput.val(new Intl.DateTimeFormat('fa-IR', {
                    // @ts-ignore
                    numberingSystem: 'latn',
            ...
            $shamsiInput.insertAfter(input);

            $shamsiInput.on("change", function (e) {
                // @ts-ignore
                const shamsiValue = e.target.value;

                const [year, month, day] = shamsiValue.split("/");
                
                // @ts-ignore
                const date = new persianDate([parseInt(year),parseInt(month),parseInt(day)]);

                const enDate = date.toDate();

                const enYear = enDate.getFullYear();
                const enMonth = (enDate.getMonth() + 1).toString().padStart(2,"0");
                const enDay = enDate.getDate().toString().padStart(2,"0");

                input.val(`${enYear}/${enMonth}/${enDay}`);
            ...
            // @ts-ignore
            kamaDatepicker(jalaliId, {markHolidays:true,markToday:true, highlightSelectedDay:true,sync:true, twodigit: false});
        }
        else {
