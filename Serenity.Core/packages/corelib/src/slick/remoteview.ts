...
export class RemoteView<TEntity> {
    constructor(options: RemoteViewOptions) {
        ...
        function addData(data: any) {
            ...
            generateCardView(data.Entities);
        ...
        let cardConfig = null as any;

        function setCardConfig (config:any){
            cardConfig = config;
        }

        function generateCardView(data:any)
        {
            ...
            for (let i = 0; i < data.length; i++) {
                const item = data[i];
                const element = `<div class="col-md-3"><div class="card">
                <img class="card-img-top" src="${item.cardImage}">
                <div class="card-body">
                  <h5 class="card-title">${item[cardConfig.title]}</h5>
                  <p class="card-text">${item[cardConfig.remark]}</p>
                  <a class="btn btn-primary" onclick="editItem(${item[cardConfig.id]})">${localText('Controls.SelectEditor.InplaceEdit')}</a>
                </div>
              </div></div>`;

              $(element).appendTo(cardContainer);
            }
        }
